...
All hooks return `null` or partial objects during initial load:

```tsx
const batteryLevel = useBatteryLevel();

// Always handle null (loading) state
if (batteryLevel === null) {
  return <LoadingSpinner />;
}
```

### 2. Memoize Dependent Components

Prevent unnecessary re-renders with `React.memo`:

```tsx
const BatteryIcon = React.memo(({ level }: { level: number }) => {
  // Only re-renders when level actually changes
...
Some hooks have platform-specific behavior:
...
function BrightnessControl() {
  ...
  if (Platform.OS === 'android') {
    // Brightness hook not supported on Android
    return null;
  }
...
### 4. Combine with Other React Hooks

Hooks work naturally with `useEffect` and `useState`:

```tsx
function SmartBatteryWidget() {
  const batteryLevel = useBatteryLevel();
  const powerState = usePowerState();
  ...
  useEffect(() => {
    if (batteryLevel !== null && batteryLevel < 0.1) {
      setShowWarning(true);
    }
  }, [batteryLevel]);
...
function AudioControls() {
  ...
  return (
    <View>
      {headphonesConnected && (
        <Text>🎧 Headphones detected - enjoy your music!</Text>
      )}
      <VolumeSlider />
    </View>
  );
